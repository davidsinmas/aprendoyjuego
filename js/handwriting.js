(()=>{
  ...
  function templateCanvas(letter,font,weight=400,mode='fill'){const c=document.createElement('canvas');c.width=180;c.height=180;const x=c.getContext('2d');x.font=`${weight} 132px ${font}`;x.textAlign='center';x.textBaseline='middle';x.fillStyle='#000';x.strokeStyle='#000';x.lineJoin='round';if(mode==='fill')x.fillText(letter,90,94);else{x.lineWidth=weight===400?10:7;x.strokeText(letter,90,94);}return c;}
  function ensureTemplates(){if(hw.templates)return hw.templates;const fonts=['Arial','Trebuchet MS','Verdana','Avenir Next'];hw.templates={};for(const letter of ALPHABET){const variants=[];const forms=[letter,letter.toLocaleLowerCase('es-ES')];for(const form of forms)for(const font of fonts)for(const weight of [400,700])for(const mode of ['fill','stroke']){const mask=normalizedMask(templateCanvas(form,font,weight,mode));if(mask)variants.push(mask);}hw.templates[letter]=variants;}return hw.templates;}
  ...
  function acceptanceLimits(){const level=Number(state.level?.level||1);if(level<=6)return{maxDistance:5.25,strongDistance:3.8,maxRank:9,margin:1.45};if(level<=16)return{maxDistance:5.05,strongDistance:3.6,maxRank:8,margin:1.3};return{maxDistance:4.8,strongDistance:3.4,maxRank:7,margin:1.15};}
  function isExpectedLetter(result,expected){if(!result?.ranking?.length)return false;const limits=acceptanceLimits(),rank=result.ranking.findIndex(item=>item.letter===expected);if(rank<0)return false;const target=result.ranking[rank],best=result.ranking[0];if(!Number.isFinite(target.distance)||target.distance>limits.maxDistance)return false;if(target.distance<=limits.strongDistance)return true;return rank<limits.maxRank&&target.distance<=best.distance+limits.margin;}
