...
(function(){
  ...
  if(!supabaseClient){
    console.warn('[Ludeiko] No se pudo inicializar Supabase. El juego seguirá funcionando en local.');
    return;
  }
  ...
  function setStatus(message,isError=false){
    ...
    if(el){el.textContent=message;el.dataset.error=isError?'1':'0';}
  }
  ...
  function bindAccountEvents(root){
    ...
    if(signup&&!signup.dataset.bound){
      ...
      signup.addEventListener('click',async()=>{
        const email=form?.email?.value.trim()||'';
        const password=form?.password?.value||'';
        if(!email||password.length<6){setStatus('Introduce un correo y una contraseña de al menos 6 caracteres.',true);return;}
        setStatus('Creando cuenta…');
        const {data,error}=await supabaseClient.auth.signUp({email,password,options:{data:{product:'ludeiko'}}});
        if(error){setStatus(error.message,true);return;}
        if(data.session)setStatus('Cuenta creada. Sincronización activa.');
        else setStatus('Cuenta creada. Revisa tu correo para confirmar la cuenta y después inicia sesión.');
      });
    }
