...
(function(){
  ...
  async function restore(session){
    ...
    try{
      ...
      if(l>m&&l>r){await upload(true);status('Sincronizado');return;}
      if(r>m||!lastLoaded){
        D=remote;
        if(typeof D==='object')D.versionDatos=Math.max(DATA_VERSION,Number(D.versionDatos)||DATA_VERSION);
        localSave(D);
  ...
  function processSession(session,event){
    if(event==='SIGNED_OUT'){localStorage.removeItem(LOAD_MARKER);localStorage.removeItem(DIRTY_MARKER);lastSessionId='';render();return;}
    if(!session)return;
