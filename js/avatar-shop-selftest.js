...
(function(){
  ...
  function run(){
    ...
    Promise.all((AVATAR.items.filter(i=>i.available!==false)).map(i=>AvatarSystem.inspectImageDimensions(i.avatarLayer))).then(assetChecks=>{
      const failed=assetChecks.filter(x=>!x.ok);
      results.push(report('Assets paper-doll',failed.length===0,failed.length?failed.map(x=>x.src).join(', '):`${assetChecks.length} assets válidos.`));
