/* V3.8.12 — desbloqueo manual desde Padres.
 *
 ...
 * al iniciar esa partida el acceso se consume y vuelve a quedar bloqueado.
 */
(function(){
  ...
  /* En niveles, el desbloqueo diario normal sigue funcionando fuera de Padres. */
  ...
  /* Sustituye las tarjetas de acción para que solo permitan entrar mediante
     nuestro flujo, que consume el pase al iniciar la partida. */
  window.guardianDuelCard=function(){
    const unlocked=actionAvailable();
    const note=parentMode
      ? (unlocked?'Desbloqueado manualmente · pulsa para jugar':'Bloqueado · pulsa «Desbloquear» en Padres')
  ...
  /* El panel Padres conserva todas sus opciones y añade el pulsador explícito. */
  ...
  window.parentDashboard=function(){
    ...
    grid.insertAdjacentHTML('afterbegin',`<div class="parent-card parent-action-unlock"><h3>⚡ Juegos de acción</h3><p class="muted">Los juegos permanecen bloqueados incluso en modo Padres hasta que pulses el botón. El desbloqueo permite <b>una sola partida</b>; después vuelve a bloquearse.</p><button type="button" class="btn ${unlocked?'secondary':'primary'}" onclick="parentUnlockActionGames()">${unlocked?'✓ DESBLOQUEADO':'🔓 DESBLOQUEAR UNA PARTIDA'}</button>${unlocked?'<small class="muted">Ya está desbloqueado. Entra en Duelo de Guardianes o Defensa del planeta para consumirlo.</small>':''}</div>`);
  };

  /* Si se entra en Padres sin haber pulsado el botón, no se hereda acceso. */
  if(!parentMode){
