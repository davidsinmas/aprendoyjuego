...
(() => {
  ...
  function hookHome(){
    injectStyle();
    if(typeof window.home!=='function' || window.home.__tankPixelHooked) return;
  ...
  function startTank(){
    ...
    function fire(fromPlayer=true){const t=fromPlayer?player:enemy,dir=fromPlayer?-1:1;bullets.push({x:t.x,y:t.y+dir*22,r:7,vx:0,vy:dir*430,owner:fromPlayer?'p':'e',bounces:0});}
    ...
    function updateBullet(b,dt){
      ...
      if(b.x+b.r>=W){b.x=W-b.r;b.vx=-Math.abs(b.vx);b.bounces++;}
    ...
    function draw(){
      ctx.fillStyle='#0c1320';ctx.fillRect(0,0,W,H);ctx.fillStyle='#111d2b';for(let x=0;x<W;x+=32)for(let y=0;y<H;y+=32)if((x/32+y/32)%2===0)ctx.fillRect(x,y,1,1);ctx.strokeStyle='#26384a';ctx.lineWidth=4;ctx.strokeRect(2,2,W-4,H-4);
      obstacles.forEach(o=>{if(!o.dead)drawObstacle(o);});if(enemy.alive)drawPixelTank(enemy,false);drawPixelTank(player,true);
      for(const b of bullets){ctx.fillStyle=b.owner==='p'?'#ffe16b':'#ff7070';ctx.fillRect(Math.round(b.x-b.r),Math.round(b.y-b.r),b.r*2,b.r*2);ctx.fillStyle='#fff7c2';ctx.fillRect(Math.round(b.x-2),Math.round(b.y-2),4,4);}
      for(const s of sparks){ctx.fillStyle='#ffd65c';ctx.fillRect(s.x-2,s.y-2,4,4);}const scoreEl=document.querySelector('.tank-score');if(scoreEl)scoreEl.textContent='💥 '+score;
      if(!running){ctx.fillStyle='rgba(6,10,16,.82)';ctx.fillRect(0,0,W,H);ctx.textAlign='center';ctx.fillStyle='#fff';ctx.font='bold 42px monospace';ctx.fillText(enemy.alive?'¡Fin de la partida!':'¡Tanque enemigo destruido!',W/2,H/2-20);ctx.font='bold 24px monospace';ctx.fillText('Puntuación: '+score,W/2,H/2+28);ctx.font='18px monospace';ctx.fillText('Pulsa 🔄 para repetir',W/2,H/2+68);}
    }
