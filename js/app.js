...
function parentGameSettingsHTML(){
  ...
  return `<div class="parent-game-list">${GAME_CONTROLS.map(game=>{const active=settings[game.type]!==false;return `<button type="button" class="parent-option-row parent-game-toggle ${active?'on':'off'}" onclick="toggleGame('${game.type}')" aria-pressed="${active}" aria-label="${game.label}: ${active?'activado':'desactivado'}"><span>${game.icon} ${game.label}</span><span class="parent-switch-ui" aria-hidden="true"><i></i></span></button>`;}).join('')}</div>`;
}
...
function openPlanetDefense(){
  const controller=window.PlanetDefense;
  if(controller&&typeof controller.start==='function'){controller.start();return;}
...
function dailyLevel(type){
  const levels=GAME.levels[type]||[];
  if(!levels.length)return null;
  let index=0;
  for(let i=0;i<levels.length;i++){
    if(stats(levels[i].id).partidas>0)index=Math.min(i+1,levels.length-1);
    else break;
  }
  return levels[index];
}
...
function markDaily(type){
  ...
  if(D.retosDiarios.retos.every(r=>r.done)&&!D.retosDiarios.premio){
    ...
    unlockActionGames('daily');
    D.diamantes+=10;
    giveXP(10);
  }
  save(D);
}
function finishDailyActivity(type,title='¡Reto diario completado!'){
  const perfect=state.hits===state.total,bonus=perfect?5:3,xp=5+(perfect?5:0);
  D.diamantes+=bonus;giveXP(xp);markDaily(type);checkAchievements();save(D);
  layout(`<div class="top"><h2>${title}</h2>${diamond(true)}</div><div class="question score">${state.hits} de ${state.total}</div><p class="center reward-line">+${bonus} 💎 · +${xp} XP</p><button class="btn primary" onclick="home()">Volver a los retos</button>`);
  window.GameSound?.play('dailyComplete');
}
...
<h3 class="section-title">Unidades pedagógicas</h3>
${pedagogyHomeCard()}
...
function collectionName(item){return AVATAR.collections?.[item.collection]?.name||'Colección';}
function avatarCollectionStats(collection='all'){
  const items=collection==='all'?avatarCatalog():avatarCatalog(null,collection),avatar=AvatarSystem.ensureState(D),owned=items.filter(item=>avatar.owned.includes(item.id)).length;
  return{owned,total:items.length,equipped:AvatarSystem.equippedItems(D).length};
}
function collectionProgress(collection='all'){
  const stats=avatarCollectionStats(collection),pct=stats.total?Math.round(stats.owned/stats.total*100):0,name=collection==='all'?'Todas las colecciones':(AVATAR.collections?.[collection]?.name||'Colección');
  return `<div class="collection-progress"><div class="collection-progress-head"><div><span class="eyebrow">Colección</span><b>${escapeHTML(name)}</b></div><strong>${stats.owned}/${stats.total}</strong></div><div class="collection-track" aria-label="${stats.owned} de ${stats.total} objetos conseguidos"><span style="width:${pct}%"></span></div><small>${stats.owned===stats.total?'¡Conjunto completo!':stats.total-stats.owned+' piezas por conseguir'}</small></div>`;
}
...
function shopItemCard(item,selected){
  ...
  return `<button type="button" class="shop-card collection-${item.collection||'general'} rarity-border-${item.rarity} ${selected?'selected':''} ${owned?'owned':''} state-${stateInfo.kind}" onclick="selectShopItem('${item.id}')" aria-pressed="${selected}"><span class="shop-icon">${shopVisual(item)}</span><span class="shop-info"><span class="shop-title"><b>${escapeHTML(item.name)}</b><span class="rarity rarity-${item.rarity}">${escapeHTML(rarityLabel(item))}</span></span><span class="shop-slot">${escapeHTML(collectionName(item))} · ${escapeHTML(AVATAR.slots[item.slot]?.label||item.slot)} · ${tierLabel(item)}</span><span class="shop-card-state shop-state-${stateInfo.kind}">${stateInfo.label}</span></span></button>`;
}
function shopDetail(item){
  ...
  return `<div class="shop-detail"><div class="shop-detail-title"><div><span class="eyebrow">${escapeHTML(collectionName(item))} · ${escapeHTML(AVATAR.slots[item.slot]?.label||item.slot)}</span><h2>${escapeHTML(item.name)}</h2></div><span class="rarity rarity-${item.rarity}">${escapeHTML(rarityLabel(item))}</span></div><p>${escapeHTML(item.description||'')}</p>${action}${!owned?'<small>Al comprarla se equipará automáticamente.</small>':'<small>La pieza seguirá guardada en tu inventario.</small>'}</div>`;
}
...
function rewardProgressCorrect(sound='correct'){D.totalAciertos++;giveXP(2);save(D);window.GameSound?.play(sound);}
...
function startCompare(n,daily=false){
  const total=daily?5:GAME.compareTotal;
  const targets=mix(Array.from({length:total},(_,i)=>i<Math.ceil(total/2)?'mayor':'menor'));
  const qs=targets.map(target=>{const [a,b]=makeComparePair(n);return{a,b,target,r:target==='mayor'?Math.max(a,b):Math.min(a,b)};});
  state={...state,type:'comparar',level:n,mode:null,qs,i:0,hits:0,daily,total};
  compareQuestion();
}
...
function pickSyllable(index){
  ...
  if(ok){
    state.locked=true;
    if(!state.questionHadError){state.hits++;rewardProgressCorrect();}else window.GameSound?.play('corrected');
...
function finishReading(){
  ...
  window.GameSound?.play(!wasDone&&nextLevelButton(type,n)?'advance':'levelComplete');
}
...
function parentDashboard(){
  const largeSubtractions=D.ajustes.restasMayoresDe10;
  ...
  <section class="parent-section" data-parent-section="player"><h3>👤 Jugador y progreso</h3><div class="parent-section-body">
    <div class="parent-card parent-profile"><label>Nombre del jugador</label><div class="parent-inline-control"><input id="nameInput" type="text" maxlength="24" value="${escapeHTML(D.perfil.nombre)}"><button class="btn secondary" onclick="setName()">Guardar</button></div></div>
    <div class="parent-card"><div class="parent-compact-fields"><label>Nivel<input id="parentLevel" type="number" min="1" max="999" step="1" value="${D.nivelJugador}"></label><label>XP<input id="parentXP" type="number" min="0" step="1" value="${D.xp}"></label></div><button class="btn secondary" onclick="setParentProgress()">Aplicar nivel y XP</button></div>
    <div class="parent-card"><div class="parent-option-heading"><b>💎 Diamantes</b><span>${D.diamantes}</span></div><div class="parent-button-row"><button class="small secondary" onclick="changeDiamonds(-100)">−100</button><button class="small secondary" onclick="changeDiamonds(100)">+100</button><button class="small secondary" onclick="changeDiamonds(500)">+500</button></div><div class="parent-inline-control"><input id="parentDiamonds" aria-label="Cantidad exacta de diamantes" type="number" min="0" step="1" value="${D.diamantes}"><button class="btn secondary" onclick="setDiamondsExact()">Aplicar</button></div></div>
  </div></section>
  <section class="parent-section" data-parent-section="learning"><h3>📚 Aprendizaje</h3><div class="parent-section-body">
    <div class="parent-card"><button type="button" class="parent-option-row parent-toggle-button ${largeSubtractions?'on':'off'}" onclick="toggleLargeSubtractions()" aria-pressed="${largeSubtractions}"><span class="parent-option-copy"><b>Restas con números mayores de 10</b><small>Recomendado después de «La misión del puente del 10».</small></span><span class="parent-switch-ui" aria-hidden="true"><i></i></span></button></div>
    <div class="parent-card"><div class="parent-option-heading"><b>Juegos educativos visibles</b><small>También se aplicará a los retos diarios.</small></div>${parentGameSettingsHTML()}</div>
  </div></section>
  <section class="parent-section" data-parent-section="actions"><h3>🧪 Pruebas y recompensas</h3><div class="parent-section-body">
    <div class="parent-card"><div class="parent-button-row"><button class="btn secondary" onclick="parentTestLevels()">Probar niveles</button><button class="btn secondary" onclick="startGuardianDuel()">Probar duelo</button><button class="btn secondary" onclick="openPlanetDefense()">Probar defensa</button></div></div>
    <div class="parent-card"><div class="parent-option-heading"><b>🛍️ Precio de la tienda</b><small>No modifica las compras ya realizadas.</small></div><div class="parent-button-row"><button class="small secondary" onclick="setShopPriceMultiplier(0.5)">×0,5</button><button class="small secondary" onclick="setShopPriceMultiplier(1)">×1</button><button class="small secondary" onclick="setShopPriceMultiplier(1.5)">×1,5</button><button class="small secondary" onclick="setShopPriceMultiplier(2)">×2</button></div><div class="parent-inline-control"><input id="shopPriceMultiplier" aria-label="Multiplicador del precio de la tienda" type="number" min="0.25" max="3" step="0.25" value="${D.ajustes.multiplicadorPrecios}"><button class="btn secondary" onclick="setShopPriceMultiplier()">Aplicar</button></div><small class="muted">Rango: ${Math.min(...avatarCatalog().map(avatarItemPrice))}–${Math.max(...avatarCatalog().map(avatarItemPrice))} 💎</small></div>
  </div></section>
  <section class="parent-section" data-parent-section="account"><h3>☁️ Cuenta y datos</h3><div class="parent-section-body">
    <div class="parent-card"><div class="parent-button-row"><button class="btn secondary" onclick="exportData()">Guardar copia</button><label class="btn secondary file-label" for="importFile">Recuperar copia</label><input id="importFile" type="file" accept=".json,application/json" onchange="importData(event)" hidden></div></div>
    <div class="parent-card parent-security-actions"><button class="btn secondary" onclick="parentPinSetup(true)">Cambiar PIN</button><button class="btn danger" onclick="disableParentMode()">Quitar modo Padres</button><button class="btn danger" onclick="resetAll()">Borrar todo el progreso</button></div>
  </div></section>
  </div>`);
}
